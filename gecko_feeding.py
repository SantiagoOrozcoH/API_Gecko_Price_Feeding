...
def check_alerts(price):
    ...
    for i in range(len(ion_price_alerts)):
        if((price > (offset_ion_price+ion_price_alerts[i])) & ion_price_alerts_pflags[i]):
            ...
            body = "$"+str(round(price,2))+currency+"\n\nToday's Offset: "+str(offset_ion_price)+currency+" ---> +"+str(round(percent,2))+"%"
...
url_ion_price = "https://api.coingecko.com/api/v3/simple/price?ids=ion&vs_currencies=usd"
...
cryptos =["osmosis", "cosmos", "ion"]
currency = "USD"

offset_ion_price = price_extract(url_ion_price, "ion", str.lower(currency))                        #change for todays price
hour_max_ion_price = offset_ion_price
hour_min_ion_price = offset_ion_price
...
while(1):
    ...
    for crypt in cryptos:
        url_price = "https://api.coingecko.com/api/v3/simple/price?ids="+crypt+"&vs_currencies="+currency
        try:
            p = price_extract(url_price, crypt, str.lower(currency))

            print("{:15s} {:10s} {:4.2f} \t {}".format(crypt,"Price: ", p, currency))

            #print("{:20s} {:4.1f} <--- {}".format(crypt+"Price: ", p))

            #print(crypt,"\t\tPrice: \t", round(p,2), currency)
